...
function showAdminPage(page) {
  ...
  else if (page === 'kasa-kalemleri') renderKasaKalemleri();
...
async function renderSifreler() {
  const m = document.getElementById('main-content');
  m.innerHTML = `<div class="page-header"><div class="page-title"><div class="icon-wrap"><i class="fa-solid fa-key"></i></div>Şifre Yönetimi</div></div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px" id="sifre-grid">
      <div class="card"><div class="card-title"><i class="fa-solid fa-spinner fa-spin"></i> Yükleniyor...</div></div>
    </div>`;

  // Personel şifre ayarını çek
  let personelAyar = { sifre: '5586Persona', aktif: true };
  try { personelAyar = await adminApi('GET', '/personel-sifre-ayar'); } catch(e) {}

  document.getElementById('sifre-grid').innerHTML = `
    <!-- Admin Şifresi -->
    <div class="card">
      <div class="card-title"><i class="fa-solid fa-shield-halved" style="color:var(--red)"></i> Admin Şifresi</div>
      <p style="font-size:13px;color:var(--muted);margin-bottom:16px">Admin paneline giriş şifresi</p>
      <div class="form-group">
      ...
      <div class="form-actions">
        <button class="btn btn-danger" onclick="sifreDegistir('admin')">
          <i class="fa-solid fa-key"></i> Değiştir
        </button>
      </div>
    </div>
    ...
    <!-- İÇDER Şifresi -->
    <div class="card">
      <div class="card-title"><i class="fa-solid fa-user" style="color:var(--accent)"></i> İÇDER Giriş Şifresi</div>
      <p style="font-size:13px;color:var(--muted);margin-bottom:16px">İÇDER kullanıcı giriş ekranı şifresi</p>
      <div class="form-group">
    ...
    <!-- Personel Detay Şifresi -->
    <div class="card" style="grid-column:span 2">
      <div class="card-title"><i class="fa-solid fa-id-card" style="color:#f59e0b"></i> Personel Detay Şifresi</div>
      <p style="font-size:13px;color:var(--muted);margin-bottom:16px">
        Personel detaylarını (TC, IBAN, maaş vb.) görüntülemek için gereken şifre. Devre dışı bırakırsanız şifre sorulmaz.
      </p>
      <div style="display:flex;align-items:center;gap:16px;margin-bottom:20px;background:var(--bg2);border-radius:10px;padding:14px 18px;border:1px solid var(--border)">
        <div style="flex:1">
          <div style="font-weight:700;font-size:14px;color:var(--text)">Şifre Koruması</div>
          <div style="font-size:12px;color:var(--muted);margin-top:2px">${personelAyar.aktif ? '🔒 Aktif — Detay görüntülemek için şifre gerekiyor' : '🔓 Devre Dışı — Şifresiz erişim açık'}</div>
        </div>
        <label style="display:flex;align-items:center;gap:10px;cursor:pointer">
          <span style="font-size:13px;color:var(--muted)">${personelAyar.aktif ? 'Aktif' : 'Pasif'}</span>
          <div style="position:relative;width:48px;height:26px" onclick="personelSifreToggle(this)">
            <input type="checkbox" id="personel-sifre-toggle" ${personelAyar.aktif ? 'checked' : ''} style="opacity:0;position:absolute;width:100%;height:100%;cursor:pointer;margin:0;z-index:2" onchange="personelSifreAktifDegistir(this.checked)" />
            <div style="position:absolute;inset:0;border-radius:13px;background:${personelAyar.aktif ? 'var(--accent)' : 'var(--border)'};transition:background .2s"></div>
            <div style="position:absolute;top:3px;left:${personelAyar.aktif ? '25px' : '3px'};width:20px;height:20px;border-radius:50%;background:#fff;transition:left .2s;box-shadow:0 1px 4px rgba(0,0,0,.3)"></div>
          </div>
        </label>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
        <div class="form-group">
          ...
          <input type="text" value="${personelAyar.sifre}" readonly style="opacity:.6;cursor:default" />
        </div>
        <div class="form-group">
          <label>Yeni Şifre</label>
          <input id="personel-sifre-input" type="password" placeholder="Yeni şifre girin"/>
        </div>
      </div>
      <div class="form-actions">
        <button class="btn btn-warning" onclick="sifreDegistir('personel')" style="background:#f59e0b;border-color:#f59e0b">
          <i class="fa-solid fa-key"></i> Personel Şifresini Değiştir
        </button>
      </div>
    </div>
  `;
...
async function sifreDegistir(tur) {
  ...
  if (!yeniSifre) { toast('Lütfen yeni şifre girin', 'error'); return; }
  if (yeniSifre.length < 4) { toast('Şifre en az 4 karakter olmalı', 'error'); return; }
  try {
    await adminApi('POST', '/sifre-degistir', { tur, yeni_sifre: yeniSifre });
    const label = tur === 'admin' ? 'Admin' : tur === 'icder' ? 'İÇDER' : 'Personel';
    toast(`${label} şifresi başarıyla değiştirildi`);
    input.value = '';
    if (tur === 'personel') renderSifreler(); // sayfayı yenile ki mevcut şifre güncellensin
  } catch (e) { toast(e.message, 'error'); }
}

async function personelSifreAktifDegistir(aktif) {
  try {
    await adminApi('POST', '/personel-sifre-aktif', { aktif });
    toast(aktif ? 'Personel şifre koruması aktif edildi' : 'Personel şifre koruması devre dışı bırakıldı');
    renderSifreler();
  } catch(e) { toast(e.message, 'error'); }
}
...
// ─── KASA KALEMLERİ YÖNETİMİ ─────────────────────────────────────────────────
async function renderKasaKalemleri() {
  const m = document.getElementById('main-content');
  m.innerHTML = `
    <div class="page-header">
      <div class="page-title"><div class="icon-wrap"><i class="fa-solid fa-tags"></i></div>Kasa Kalemleri</div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px" id="kasa-kalemleri-grid">
      <div class="card"><div class="card-title"><i class="fa-solid fa-spinner fa-spin"></i> Yükleniyor...</div></div>
    </div>`;

  let giderler = [], gelirler = [];
  try {
    [giderler, gelirler] = await Promise.all([
      adminApi('GET', '/kasa-gider-kalemleri'),
      adminApi('GET', '/kasa-gelir-kalemleri'),
    ]);
  } catch(e) {}

  document.getElementById('kasa-kalemleri-grid').innerHTML = `
    <!-- GİDER KALEMLERİ -->
    <div class="card">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
        <div class="card-title" style="margin:0"><i class="fa-solid fa-arrow-up" style="color:#ef4444;margin-right:8px"></i>Gider Kalemleri</div>
        <button class="btn btn-sm btn-danger" onclick="modalKalemEkle('gider')"><i class="fa-solid fa-plus"></i> Ekle</button>
      </div>
      ${giderler.length === 0 ? '<div class="empty-state" style="padding:30px"><i class="fa-solid fa-tags"></i><p>Kalem yok</p></div>' : `
        <div style="display:flex;flex-direction:column;gap:8px">
          ${giderler.map(k => `
            <div style="display:flex;align-items:center;justify-content:space-between;background:var(--bg2);border-radius:9px;padding:11px 14px;border:1px solid var(--border)">
              <div>
                <div style="font-weight:700;font-size:13px;color:var(--text)">${k.ad}</div>
                <div style="font-size:11px;color:var(--muted)">${k.kategori||'Genel'} ${k.varsayilan_tutar?'· '+Number(k.varsayilan_tutar).toLocaleString('tr-TR')+' ₺':''}</div>
              </div>
              <div style="display:flex;gap:6px">
                <button class="btn btn-sm btn-secondary" onclick="modalKalemDuzenle('gider',${k.id},'${k.ad.replace(/'/g,"\\'")}','${k.kategori||''}',${k.varsayilan_tutar||0})"><i class="fa-solid fa-pen"></i></button>
                <button class="btn btn-sm btn-danger" onclick="kalemSil('gider',${k.id})"><i class="fa-solid fa-trash"></i></button>
              </div>
            </div>
          `).join('')}
        </div>
    ...
    <div class="card">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
        <div class="card-title" style="margin:0"><i class="fa-solid fa-arrow-down" style="color:#10b981;margin-right:8px"></i>Gelir Kalemleri</div>
        <button class="btn btn-sm btn-primary" onclick="modalKalemEkle('gelir')"><i class="fa-solid fa-plus"></i> Ekle</button>
      </div>
      ${gelirler.length === 0 ? '<div class="empty-state" style="padding:30px"><i class="fa-solid fa-tags"></i><p>Kalem yok</p></div>' : `
        <div style="display:flex;flex-direction:column;gap:8px">
          ${gelirler.map(k => `
            <div style="display:flex;align-items:center;justify-content:space-between;background:var(--bg2);border-radius:9px;padding:11px 14px;border:1px solid var(--border)">
              <div>
              ...
              <div style="display:flex;gap:6px">
                <button class="btn btn-sm btn-secondary" onclick="modalKalemDuzenle('gelir',${k.id},'${k.ad.replace(/'/g,"\\'")}','${k.kategori||''}',${k.varsayilan_tutar||0})"><i class="fa-solid fa-pen"></i></button>
                <button class="btn btn-sm btn-danger" onclick="kalemSil('gelir',${k.id})"><i class="fa-solid fa-trash"></i></button>
              </div>
            </div>
          `).join('')}
        </div>
...
function modalKalemEkle(tur) {
  const renk = tur === 'gider' ? '#ef4444' : '#10b981';
  const baslik = tur === 'gider' ? '↑ Gider Kalemi Ekle' : '↓ Gelir Kalemi Ekle';
  openModal(baslik, `
    <div class="form-group"><label>Kalem Adı *</label><input id="km-ad" placeholder="Örn: Ofis Kirası" /></div>
    <div class="form-group"><label>Kategori</label>
      <select id="km-kategori">
        ${tur === 'gider' ? `
    ...
    <div class="form-group"><label>Varsayılan Tutar (₺)</label><input id="km-tutar" type="number" placeholder="0" /></div>
    <div class="form-actions">
      <button class="btn btn-secondary" onclick="closeModal()">İptal</button>
      <button class="btn" style="background:${renk};color:#fff;border-color:${renk}" onclick="kalemKaydet('${tur}')"><i class="fa-solid fa-check"></i> Kaydet</button>
    </div>
  `);
}

function modalKalemDuzenle(tur, id, ad, kategori, tutar) {
  const renk = tur === 'gider' ? '#ef4444' : '#10b981';
  openModal('Kalemi Düzenle', `
    <div class="form-group"><label>Kalem Adı *</label><input id="km-ad" value="${ad}" /></div>
    <div class="form-group"><label>Kategori</label><input id="km-kategori" value="${kategori}" /></div>
    <div class="form-group"><label>Varsayılan Tutar (₺)</label><input id="km-tutar" type="number" value="${tutar}" /></div>
    <div class="form-actions">
      <button class="btn btn-secondary" onclick="closeModal()">İptal</button>
      <button class="btn" style="background:${renk};color:#fff;border-color:${renk}" onclick="kalemGuncelle('${tur}',${id})"><i class="fa-solid fa-check"></i> Güncelle</button>
    </div>
  `);
}

async function kalemKaydet(tur) {
  const ad = document.getElementById('km-ad')?.value?.trim();
  const kategori = document.getElementById('km-kategori')?.value;
  const varsayilan_tutar = parseFloat(document.getElementById('km-tutar')?.value) || 0;
  if (!ad) return toast('Kalem adı zorunlu', 'error');
  try {
    const endpoint = tur === 'gider' ? '/kasa-gider-kalemleri' : '/kasa-gelir-kalemleri';
    await adminApi('POST', endpoint, { ad, kategori, varsayilan_tutar });
    closeModal();
    toast('Kalem eklendi');
    renderKasaKalemleri();
  } catch(e) { toast(e.message, 'error'); }
...
async function kalemGuncelle(tur, id) {
  const ad = document.getElementById('km-ad')?.value?.trim();
  const kategori = document.getElementById('km-kategori')?.value;
  const varsayilan_tutar = parseFloat(document.getElementById('km-tutar')?.value) || 0;
  if (!ad) return toast('Kalem adı zorunlu', 'error');
  try {
    const endpoint = tur === 'gider' ? '/kasa-gider-kalemleri/' : '/kasa-gelir-kalemleri/';
    await adminApi('PUT', endpoint + id, { ad, kategori, varsayilan_tutar });
    closeModal();
...
async function kalemSil(tur, id) {
